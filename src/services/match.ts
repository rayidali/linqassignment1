...
- requested_music_query: put a SHORT search query here in TWO cases: (1) the user explicitly asks for music (a specific song, artist, genre, or vibe like "upbeat", "lofi", "sad piano", "use Hot in Herre by Nelly") — distill it to a genre/mood; we only have royalty-free music, so famous songs aren't available ("use Hot in Herre by Nelly" → "upbeat 2000s hip hop instrumental"); OR (2) the caption names a theme/occasion that implies a clear musical mood — "birthday edit" → "happy birthday celebration instrumental", "wedding" → "romantic wedding piano instrumental", "graduation" → "triumphant uplifting instrumental", "halloween" → "spooky eerie instrumental", "christmas" → "festive christmas instrumental", "workout" → "high energy gym instrumental", etc. Add "instrumental" when it fits. If neither applies (just a generic vibe with no music/theme cue), set requested_music_query to null and the template's default track is used.
