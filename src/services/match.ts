...
• text_overlays — on-screen text, as an array (usually 1-3 items; 4 max — don't overload):
   ...
   - DEFAULT BIAS: a designed edit usually has at least one title. If the user gave any meaningful caption ("a trip to new york", "summer 2026", "first day of college") add at least ONE title overlay drawn from it — even if they didn't explicitly ask. For a polished feel, pair a HERO with a SUBTITLE at the SAME position so they stack as a designed unit (the renderer composes them into one HTML block — the magazine "BIG TITLE / smaller tagline" look). Skip overlays only for: explicit "no text", clean cinematic/aesthetic vibes where text would clutter, or 1-clip raw cuts.
