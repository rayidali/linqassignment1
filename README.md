...
**Music (`src/services/music.ts`):** `resolveMusicUrl` resolves the `MusicSpec` against Jamendo — first a hand-curated track id for iconic themes (christmas → the real "Jingle Bells", plus halloween/summer/romantic), then a tag-driven ladder (`tags=<all>` → drop to just the genre tag → `fuzzytags`; Jamendo's loose free-text `search` is only a last resort), pulling a pool of candidates per level and picking one at random (variety), then a hardcoded CDN track as the final fallback. The chosen track is re-hosted on R2 so Shotstack can fetch it reliably (Jamendo audio URLs expire).
...
**Media normalization (`src/services/transcode.ts`):** every uploaded clip is normalized *before* it touches Shotstack — ffmpeg bakes iPhone rotation into the pixels, transcodes HEVC→H.264 (CRF 21), caps the long side at 1920px; images (incl. HEIC, via `sharp`) are auto-rotated from EXIF and looped into a short clip. The first clip's dimensions size the render output (so a portrait source gives a portrait video), clamped to fit Shotstack's plan limit (long ≤ 1920, short ≤ 1080).
...
- Overlay fonts cover ~all of Google Fonts (by name, via the `@fontsource` CDN) plus a 5-category fallback — but not *proprietary* fonts (literal Helvetica, Arial, Times), which can't legally be hosted, so those map to the nearest open-license category. Wider per-glyph control (stroke widths, gradients, etc.) would mean switching overlays to Shotstack's `text` asset.
