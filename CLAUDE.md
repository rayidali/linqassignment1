...
- `text_overlays[]` — `{ text, position: top|center|bottom, color: hex/name (sanitized), uppercase, background: "none" | a color (a rounded pill behind the text), animation: none|fade|slide_up|slide_down (default `none` — the text just appears; matcher uses `fade` for soft vibes) }` — NOTE: the overlay font is fixed (no font field). The HTML asset's CSS `@font-face`s Montserrat ExtraBold from a CDN; if Shotstack's renderer fetches it we get that, else it falls back to a bold-sans stack (`'Arial Black'` → Arial → Liberation Sans → …) — Helvetica isn't in Shotstack's render env, so the old `font-family:Helvetica` was silently resolving to whatever bold sans was available. A proper font *picker* (script for romantic, condensed display for hype, etc.) means switching overlays to the `text` asset + declared `timeline.fonts`.
