...
- `text_overlays[]` — `{ text, position: top|center|bottom, color, uppercase, background: "none" | a color (rounded pill), animation: none|fade|slide_up|slide_down, font_name: a specific Google Font name or "" , font: bold_sans|condensed|serif|handwritten|rounded (fallback category), size: small|medium|large, outline: none|dark|light }`. Fonts: `font_name` (if set) is slugified → `@fontsource/<slug>` CDN `@font-face` (covers ~all of Google Fonts, no API key — `namedFontFace` in `templates/index.ts`); `font` is the always-set category fallback (`FONT_SPECS` — Montserrat/Anton/Playfair/Permanent Marker/Baloo 2), and a bold-sans stack is the final fallback. Proprietary fonts (Helvetica, Times) can't be hosted → matcher leaves `font_name` "" and picks the nearest category. `size` multiplies `STYLE_PRESETS[style].fontScale` (`FONT_SIZE_FACTOR`). `outline` = CSS `-webkit-text-stroke` (`paint-order:stroke fill`); `none` keeps the default soft drop-shadow.
...
| `submitted` | poll render status; self-loop (5s) until `done`. On a render *failure* (or a 4xx at submit time), retry once with a "safe" edit — `buildEdit(plan, …, { safe: true })` strips everything not long-proven (transitions beyond cut, clip motion, text backgrounds/animations) so the user still gets a (plainer) video. If it's just *slow* (past `estimatedRenderMs` + a grace): text "taking longer than i thought" (+2 min), silently resubmit the same edit once (+4 min), give up → `failed` (+9 min). One retry total either way (`renderResubmitted` flag). | `rendered` |
