...
- `text_overlays[]` — `{ text, position: top|center|bottom, color, case_style: as_written|uppercase|lowercase, background: "none" | a color (rounded pill), animation: none|fade|slide_up|slide_down, duration_seconds: number | null, font_name: a specific Google Font name or "", font: bold_sans|condensed|serif|handwritten|rounded (fallback category), size: small|medium|large, outline: none|dark|light }`. `case_style` is honored at render via CSS `text-transform`; "as_written" leaves the matcher's text alone (user can ask "all caps"/"lowercase"). `duration_seconds`: `null` = full video (the first overlay holds throughout; subsequent overlays are sequenced 2.7s apart so they don't stack). A number (0.5–60) overrides — only set when the user asks for a specific window ("show the title for 2 seconds", "intro card 3 sec"). Fonts: `font_name` (if set) is slugified → `@fontsource/<slug>` CDN `@font-face` (covers ~all of Google Fonts, no API key — `namedFontFace` in `templates/index.ts`); `font` is the always-set category fallback (`FONT_SPECS` — Montserrat/Anton/Playfair/Permanent Marker/Baloo 2), and a bold-sans stack is the final fallback. Proprietary fonts (Helvetica, Times) can't be hosted → matcher leaves `font_name` "" and picks the nearest category. `size` multiplies `STYLE_PRESETS[style].fontScale` (`FONT_SIZE_FACTOR`). `outline` = CSS `-webkit-text-stroke` (`paint-order:stroke fill`); `none` keeps the default soft drop-shadow.
