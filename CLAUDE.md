...
|---|---|---|
| `received` | filter media parts to video/image (fail "no editable…" otherwise), normalize each **sequentially** with ffmpeg — parallel transcodes thrash the single CPU core and OOM. ffmpeg args bake rotation into pixels (autorotate on), transcode to H.264 yuv420p, cap the long side at 1920 px, AND cap the OUTPUT to the first `MAX_INPUT_SECONDS` (60s) so a long source doesn't blow the 240s ffmpeg timeout (we only use ≤12s downstream). Uses `-preset superfast` for the encode-speed win on a single-CPU instance. Per-clip try/catch in `received` — if one clip fails (timeout, codec, corrupt), continue with the survivors and text the user "had to drop N clip(s) bb"; fail the whole job only when EVERY clip failed. Upload each successful clip to R2; store the first surviving clip's normalized dims as `outputSize`; record failed clips in `result.droppedClips`. | `downloaded` |
