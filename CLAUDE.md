...
- `text_overlays[]` — `{ text, position: top|center|bottom, color: hex/name (sanitized), uppercase, background: "none" | a color (a rounded pill behind the text), animation: none|fade|slide_up|slide_down (default `none` — the text just appears; matcher uses `fade` for soft vibes) }` — NOTE: the overlay font is fixed (a bold sans-serif via the HTML asset's `font-family` — Helvetica isn't in Shotstack's render env so it resolves to Arial/Liberation Sans bold); there's no font field. A proper font picker would mean switching overlays to the `text` asset + declared `timeline.fonts`.
