...
- `text_overlays[]` — `{ text, position: top|center|bottom, color: hex/name (sanitized), uppercase, background: "none" | a color (rounded pill), animation: none|fade|slide_up|slide_down, font: bold_sans|condensed|serif|handwritten|rounded, size: small|medium|large, outline: none|dark|light }`. The `font` palette is loaded via `@font-face` from jsdelivr `@fontsource` WOFF2 (`FONT_SPECS` in `templates/index.ts`) with the bold-sans stack as fallback if a fetch fails — there's no *specific licensed* font (literal Helvetica, Times); the matcher maps a font request to the nearest category. `size` multiplies `STYLE_PRESETS[style].fontScale` (`FONT_SIZE_FACTOR`). `outline` is a CSS `-webkit-text-stroke` (`paint-order: stroke fill`); `none` keeps the default soft drop-shadow. Per-style font *defaults* aren't a thing — the matcher picks the font.
